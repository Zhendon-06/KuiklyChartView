...
internal object FinancialChartRenderer {
    fun draw(c: CanvasContext, width: Float, height: Float, s: FinancialChartSpec, start: Int, count: Int, selected: Int) {
        ...
        if (closeLine) {
            ...
            if (forecastIndices.isNotEmpty()) {
                ...
                rect(c, boundaryX, top, right - boundaryX, bottom - top, s.forecastColor.opacity(0.05f))
                ...
                forecastIndices.zipWithNext().forEach { (a, b) ->
                    ...
                    if (first != null && second != null) {
                        c.beginPath(); c.moveTo(x(a), y(first.upper)); c.lineTo(x(b), y(second.upper))
                        c.lineTo(x(b), y(second.lower)); c.lineTo(x(a), y(first.lower)); c.closePath()
            ...
            indices.filter { it >= (boundary - 1).coerceAtLeast(0) }.zipWithNext().forEach { (a, b) ->
                line(c, x(a), y(s.points[a].close), x(b), y(s.points[b].close), s.forecastColor, dashed = true)
            }
        } else if (lineMode) {
